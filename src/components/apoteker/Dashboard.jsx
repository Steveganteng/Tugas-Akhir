...
function buildRecommendations(drugs = []) {
  // produce items with { id, name, satuan, restockQty }
  return (
    drugs
      .map((d) => {
        const drugId = d.id;
        const name = d.nama;
        const satuan = d.satuan || "unit";
        const stock = Number(d.stok) || 0;
        const demand = Number(d.demand) || 100; // default target if demand unknown
        const restockQty = Math.max(0, demand - stock);

        return { drugId, name, satuan, stock, demand, restockQty };
      })
      // only show positive recommendations
...
export default function Dashboard() {
  ...
  const [restockHistory, setRestockHistory] = useState(() => loadHistoryRecords());
  ...
  useEffect(() => {
    ...
    // Only allow apoteker role to view this dashboard
    if (authState.role && authState.role !== ROLE_APOTEKER) {
      window.location.href = getHomePathForRole(authState.role);
      return;
    }
  ...
  useEffect(() => {
    setRestockHistory(loadHistoryRecords());
  ...
  const recommendations = useMemo(() => buildRecommendations(drugs), [drugs]);
  ...
  const processedRecommendations = useMemo(
    () =>
      recommendations.map((item) => ({
        ...item,
        fulfilledQty: restockStatus[item.drugId]?.fulfilledQty ?? restockStatus[item.name]?.fulfilledQty ?? 0,
        confirmedAt: restockStatus[item.drugId]?.confirmedAt ?? restockStatus[item.name]?.confirmedAt ?? null,
        lastRestockAt: restockStatus[item.drugId]?.lastRestockAt ?? restockStatus[item.name]?.lastRestockAt ?? null,
        remainingQty: Math.max(0, item.restockQty - ((restockStatus[item.drugId]?.fulfilledQty ?? restockStatus[item.name]?.fulfilledQty) ?? 0)),
      })),
  ...
  const recentInputRestocks = useMemo(() => {
    return restockHistory
      .filter((item) => ["input", "manual", "upload"].includes(item.source))
      .slice()
      .sort((a, b) => {
        const leftTime = new Date(b.restockedAt || b.tanggal || 0).getTime();
        const rightTime = new Date(a.restockedAt || a.tanggal || 0).getTime();
  ...
  const mediumItems = useMemo(
    () => pendingRecommendations.filter((item) => item.remainingQty > 0 && item.remainingQty < URGENT_RESTOCK_THRESHOLD),
    [pendingRecommendations]
  );
  ...
  const submitRestockDialog = () => {
    ...
    if (restockDialog.type === "reset") {
      setRestockStatus(resetDashboardProgress({ itemName: item.name, itemId: item.drugId, targetRestok: item.restockQty }));
      closeRestockDialog();
      return;
    }
  ...
  return (
    <ApotekerLayout
      activeNav="dashboard"
      pageTitle="Restok Obat"
      pageSubtitle="Rekomendasi restok, total unit restok, dan grafik kebutuhan per minggu."
    >
      <div style={{ width: "100%", maxWidth: 1440, margin: "0 auto" }}>
          ...
          <section
            style={{
              ...cardStyle,
              marginBottom: 24,
              borderColor: urgentItems.length > 0 ? "#7f1d1d" : mediumItems.length > 0 ? "#92400e" : "#14532d",
              background: urgentItems.length > 0 ? "#2b1212" : mediumItems.length > 0 ? "#2b1608" : "#052e16",
              padding: 28,
            }}
          >
            <div
              style={{
                ...
                fontWeight: 800,
                marginBottom: 14,
                color: urgentItems.length > 0 ? "#fecaca" : mediumItems.length > 0 ? "#fcd34d" : "#bbf7d0",
              }}
            >
              {urgentItems.length > 0
                ? "Alert: Obat yang harus segera direstok"
                : mediumItems.length > 0
                ? "Peringatan: Obat dengan kebutuhan restok (sedang)"
                : "Status aman: Tidak ada obat di atas ambang prioritas tinggi"}
            </div>

            {urgentItems.length > 0 ? (
              <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: mediumItems.length > 0 ? 12 : 0 }}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 560 }}>
              <thead>
                <tr style={{ borderBottom: "1px solid #1f2937" }}>
                  <th style={{ textAlign: "left", padding: "14px 12px", fontSize: 13, color: "#6b7280" }}>ID Obat</th>
                  <th style={{ textAlign: "left", padding: "14px 12px", fontSize: 13, color: "#6b7280" }}>Nama Obat</th>
                  <th style={{ textAlign: "left", padding: "14px 12px", fontSize: 13, color: "#6b7280" }}>Satuan</th>
                  <th style={{ textAlign: "right", padding: "14px 12px", fontSize: 13, color: "#6b7280" }}>Rekomendasi Stok</th>
              ...
              <tbody>
                {processedRecommendations.map((item) => (
                  <tr key={item.name} style={{ borderBottom: "1px solid #0f151e", opacity: item.remainingQty === 0 ? 0.75 : 1 }}>
                    <td style={{ padding: "14px 12px", fontWeight: 600, color: "#9ca3af", fontSize: 14 }}>{item.drugId}</td>
                    <td style={{ padding: "14px 12px", fontWeight: 600, color: "#e5e7eb", fontSize: 14 }}>{item.name}</td>
                    <td style={{ padding: "14px 12px", color: "#9ca3af", fontSize: 14 }}>{item.satuan}</td>
        ...
        <section style={{ ...cardStyle, marginBottom: 24, overflowX: "auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, marginBottom: 16, flexWrap: "wrap" }}>
            <div>
              <h2 style={{ margin: 0, fontSize: 20, color: "#f9fafb" }}>Data Restock dari Input</h2>
              <p style={{ margin: "6px 0 0", color: "#6b7280", fontSize: 14 }}>
                Record manual, upload, dan input aktual terbaru yang juga tersimpan di histori restok.
              </p>
            </div>
            <a
          ...
          {recentInputRestocks.length === 0 ? (
            <p style={{ margin: 0, color: "#6b7280", fontSize: 13 }}>
              Belum ada data restock dari input.
            </p>
          ) : (
            <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 760 }}>
              <thead>
                <tr style={{ borderBottom: "1px solid #1f2937" }}>
                  <th style={{ textAlign: "left", padding: "14px 12px", fontSize: 13, color: "#6b7280" }}>Tanggal</th>
                  <th style={{ textAlign: "left", padding: "14px 12px", fontSize: 13, color: "#6b7280" }}>Nama Obat</th>
                  <th style={{ textAlign: "right", padding: "14px 12px", fontSize: 13, color: "#6b7280" }}>Jumlah</th>
                  <th style={{ textAlign: "right", padding: "14px 12px", fontSize: 13, color: "#6b7280" }}>Target</th>
                  <th style={{ textAlign: "left", padding: "14px 12px", fontSize: 13, color: "#6b7280" }}>Status</th>
                  <th style={{ textAlign: "left", padding: "14px 12px", fontSize: 13, color: "#6b7280" }}>Sumber</th>
                </tr>
              </thead>
              <tbody>
                {recentInputRestocks.map((item) => (
                  <tr key={`${item.source}-${item.sourceId}`} style={{ borderBottom: "1px solid #0f151e" }}>
                    <td style={{ padding: "14px 12px", color: "#9ca3af", fontSize: 13 }}>{item.tanggal || "-"}</td>
                    <td style={{ padding: "14px 12px", fontWeight: 600, color: "#e5e7eb", fontSize: 14 }}>{item.namaObat || "-"}</td>
                    <td style={{ padding: "14px 12px", textAlign: "right", color: "#86efac", fontWeight: 700, fontSize: 14 }}>{item.jumlahRestok ?? 0}</td>
                    <td style={{ padding: "14px 12px", textAlign: "right", color: "#9ca3af", fontSize: 14 }}>{item.targetRestok ?? 0}</td>
                    <td style={{ padding: "14px 12px", color: item.status === "Selesai" ? "#4ade80" : "#fbbf24", fontWeight: 700, fontSize: 14 }}>{item.status || "-"}</td>
                    <td style={{ padding: "14px 12px", color: "#6b7280", fontSize: 13 }}>
                      {item.source === "manual" ? "Input manual" : item.source === "upload" ? "Upload file" : "Input restok"}
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            <>
              ...
              <label style={{ display: "grid", gap: 10, fontSize: 15, color: "#94a3b8" }}>
                ...
                <input
                  type="number"
                  min="1"
                  max={restockDialog.item?.remainingQty ?? 1}
